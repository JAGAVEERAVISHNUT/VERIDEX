...
const severityStyles: Record<Severity, string> = {
  low: "bg-[--color-severity-low]/15 text-[--color-severity-medium] border-[--color-severity-low]/40",
  medium: "bg-[--color-severity-medium]/10 text-[--color-severity-medium] border-[--color-severity-medium]/30",
  high: "bg-[--color-severity-high]/10 text-[--color-severity-high] border-[--color-severity-high]/30",
}

const organStatusStyles: Record<OrganStatus, { label: string; className: string }> = {
  normal: {
    label: "Normal",
    className: "bg-[--color-severity-low]/15 text-[--color-severity-medium] border-[--color-severity-low]/40",
  },
  abnormal: {
    label: "Abnormal",
    className: "bg-[--color-severity-medium]/10 text-[--color-severity-medium] border-[--color-severity-medium]/30",
  },
  critical: {
    label: "Critical",
    className: "bg-[--color-severity-high]/10 text-[--color-severity-high] border-[--color-severity-high]/30",
  },
}

const mannerStyles: Record<DeathManner, string> = {
  natural: "bg-[--color-severity-low]/15 text-[--color-severity-medium] border-[--color-severity-low]/40",
  accident: "bg-[--color-severity-medium]/10 text-[--color-severity-medium] border-[--color-severity-medium]/30",
  suicide: "bg-[--color-severity-medium]/15 text-[--color-severity-medium] border-[--color-severity-medium]/30",
  homicide: "bg-[--color-severity-high]/10 text-[--color-severity-high] border-[--color-severity-high]/30",
...
const noteTagStyles: Record<NonNullable<AnalysisSection["investigationNotes"]["notes"][number]["tag"]>, string> = {
  observation: "bg-primary/10 text-primary border-primary/25",
  hypothesis: "bg-[--color-severity-medium]/10 text-[--color-severity-medium] border-[--color-severity-medium]/30",
  "follow-up": "bg-muted text-muted-foreground border-border",
  conclusion: "bg-[--color-severity-high]/10 text-[--color-severity-high] border-[--color-severity-high]/30",
}
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-3">
      ...
      {items.map((src) => (
        <span
          key={src}
          className="inline-flex items-center gap-1 rounded-md border border-border bg-muted px-2 py-0.5 font-mono text-[10px] text-muted-foreground"
        >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-2.5">
        {data.findings.map((f) => (
          <div key={f.id} className="rounded-md border border-border bg-muted p-3.5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                  <span>{f.perimortem ? "Peri-mortem / agonal" : "Pre-mortem"}</span>
                </div>
              </div>
              <Badge variant="outline" className={cn("shrink-0 capitalize", severityStyles[f.significance])}>
                {f.significance}
              </Badge>
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-3">
        <div className="md:col-span-2 rounded-md border border-border bg-muted p-4">
          ...
          <div className="mt-3 grid gap-2.5 sm:grid-cols-2">
            <div>
              <div className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Immediate</div>
              <p className="mt-0.5 text-xs leading-relaxed text-foreground/90 text-pretty">{data.immediate}</p>
            </div>
            <div>
              <div className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Underlying</div>
              <p className="mt-0.5 text-xs leading-relaxed text-foreground/90 text-pretty">{data.underlying}</p>
            </div>
          </div>

          <div className="mt-3 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Mechanism</div>
          <p className="mt-1 text-xs leading-relaxed text-foreground/90 text-pretty">{data.mechanism}</p>
        </div>
      ...
      <div>
        ...
        <ul className="grid gap-2">
          {data.contributing.map((c, i) => (
            <li
              key={i}
              className="flex items-start gap-2 rounded-md border border-border bg-muted px-3 py-2 text-sm text-foreground/90"
            >
            ...
            {data.findings.map((f) => {
              ...
              return (
                <tr key={f.id} className="hover:bg-muted/60">
                  ...
                  <td className="px-3 py-2.5 text-xs leading-relaxed text-foreground/85 text-pretty">
                    {f.observations}
                  </td>
                </tr>
              )
...
function TissuePanel({ data }: { data: AnalysisSection["tissuePathology"] }) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-2.5">
        {data.samples.map((s) => (
          <div key={s.id} className="rounded-md border border-border bg-muted p-3.5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="text-sm font-medium text-foreground">{s.region}</div>
                {s.ageEstimate && (
                  <div className="mt-0.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                    Age estimate · {s.ageEstimate}
                  </div>
                )}
                <p className="mt-2 text-xs leading-relaxed text-foreground/85 text-pretty">{s.finding}</p>
              </div>
              <Badge variant="outline" className={cn("shrink-0 capitalize", severityStyles[s.severity])}>
                {s.severity}
              </Badge>
            </div>
          </div>
          ...
          <ul className="grid gap-2">
            {data.unansweredQuestions.map((q, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-foreground/90">
                <span className="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-[--color-severity-medium]" />
...
export function AutopsyAnalysis({ data }: { data: AutopsyCase }) {
    ...
    <Card className="border-border bg-card">
      <CardHeader className="pb-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-md bg-primary/10 ring-1 ring-primary/25">
            ...
            <div>
              ...
              <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                Extracted from ingested autopsy report
              </p>
            </div>
          </div>
          <Badge
            variant="outline"
            className="self-start border-primary/30 bg-primary/5 text-primary font-mono text-[10px] uppercase tracking-wider"
          >
      ...
      <CardContent className="space-y-4">
        <div
          ...
          className="flex flex-wrap gap-1.5 rounded-md border border-border bg-muted p-1"
        >
          {TABS.map((t) => {
            const Icon = t.icon
